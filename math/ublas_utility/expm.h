...
// Some error happens, H has elements which are NaN or infinity. 
		std::cerr << "Null INPUT : " << H <<"\n";
		throw std::runtime_error("Inconsistent null input error in expm_pad (probably inf/NaN in input).");
	}
...
// Implement Matrix Inversion
	...
	if( res != 0) {
		throw std::runtime_error("Matrix inversion error in expm_pad");
	}
