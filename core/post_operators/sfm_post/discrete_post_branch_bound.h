/*
 * discrete_post_branch_bound.h
 *
 *  Created on: Jun 21, 2011
 *      Author: ray
 */

#ifndef DISCRETE_POST_BRANCH_BOUND_H_
#define DISCRETE_POST_BRANCH_BOUND_H_
...
class discrete_post_branch_bound: public discrete_post {
public:
	typedef boost::shared_ptr<discrete_post_branch_bound<scalar_type> > ptr;
	typedef boost::shared_ptr<const discrete_post_branch_bound<scalar_type> >
			const_ptr;
	...
	discrete_post_branch_bound(double aggregation_type, double clustering) :
	...
	;
	discrete_post_branch_bound(double aggregation_type, double clustering, std::string minbrak_type,
			double inters_error) :
	...
	virtual ~discrete_post_branch_bound() {
	}
	;

	discrete_post_branch_bound* clone() {
		return new discrete_post_branch_bound<scalar_type> (*this);
	}
	;
	...
	problem_instance get_instance(const jump_constraints& j,
		...
		if (source_inv && !inst.sinv_ptr) {
			throw std::runtime_error(
					"discrete_post_sf: continuous_set type not supported as invariant");
		}
		...
		if (j.get_guard()) {
			...
			if (!inst.guard_poly_ptr) {
				throw std::runtime_error(
						"discrete_post_sf: guard type not supported");
			}
		}
	...
	void post_poly(problem_instance& inst, poly_list_type & polys,
		...
		if (inst.reset_trans_ptr) {
			...
			if (M.domain() != M.codomain()) {
				...
				if (Minput.domain().size() > 0) {
					...
					logger::copyfmt_to(ss);
					ss << Minput.domain();
					LOGGER(DEBUG5,"discrete_post_branch_bound::post","nondeterministic inputs "+ss.str());
					if (inst.sinv_ptr) {
						ss << inst.sinv_ptr;
						LOGGER(DEBUG5,"discrete_post_branch_bound::post","using source invariant "+ss.str());
			...
			if (singular || mapped_U) {
				if (singular && !mapped_U) {
					LOGGER(DEBUG4,"discrete_post_branch_bound::post","using outer poly for mapping because of non-regular jump");
				} else if (!singular && mapped_U) {
					LOGGER(DEBUG4,"discrete_post_branch_bound::post","using outer poly for mapping because of nondet. inputs");
				} else {
					LOGGER(DEBUG4,"discrete_post_branch_bound::post","using outer poly for mapping because of non-regular jump and nondet. inputs");
				}
				...
				LOGGERSW(DEBUG4,"discrete_post_branch_bound::post","applying affine map to poly in discrete_post_branch_bound");
				// transform the polys
		...
		IFLOGGER(DEBUG5) {
			...
			LOGGER(DEBUG5,"discrete_post_branch_bound::post","adding target invariant "+ss.str());
		}
	...
	continuous::continuous_set_collection post(const jump_constraints& j,
		...
		using namespace continuous;
		using namespace support_function;
		LOGGERSW(DEBUG1,"discrete_post_branch_bound::post","Discrete post with discrete_post_branch_bound");
		...
		LOGGER(DEBUG2,"discrete_post_branch_bound::post","sfm of size "+to_string(inst.sfm_set_ptr->get_size()));
		if (!inst.reset_trans_ptr->is_void()) {
			...
			if (inst.sfm_set_ptr) {
				...
				//debug: check by duplicating sfm and then computing further
				...
				//duplicating sfm--
/*
				positional_vdomain set_dom = inst.sfm_set_ptr->get_variable_ids();
				typename sfm_cont_set<scalar_type>::ptr nonconst_sfm =
						typename sfm_cont_set<scalar_type>::ptr(new sfm_cont_set<scalar_type>(
								inst.sfm_set_ptr->get_postc_prb(),inst.sfm_set_ptr->get_sfm(),inst.sfm_set_ptr->get_directions(),set_dom.get_index_to_variable_id_map()));
*/
				//---


				if (inst.guard_poly_ptr) {
					LOGGER(DEBUG5,"discrete_post_branch_bound::post","guard_poly to intersect:"+to_string(inst.guard_poly_ptr));
					list = get_intersection_interval(nonconst_sfm,
							*inst.guard_poly_ptr);
					//std::cout << "getting list from guard: " << list << std::endl;
				} else {
					throw std::runtime_error(
							"discrete_post_branch_bound: guard type not supported\n");
				}

				LOGGER(DEBUG2,"discrete_post_branch_bound::post","found "+to_string(list.size())+" intervals intersecting with guard");
				/**
				 * DEBUG
				...
				IFLOGGER(DEBUG7){
					for (intv_list_type::const_iterator it = list.begin(); it
						...
						LOGGER(DEBUG7,"discrete_post_branch_bound::post","sfm section intersecting with the guard:"
								+to_string(*it));
					}
				}
				/* Initializations */
				 ...
				 */
				LOGGER(DEBUG7,"discrete_post_branch_bound::post","Target Invariant:"+to_string(inst.tinv_ptr));

				LOGGER(DEBUG7,"discrete_post_branch_bound::post","guard cons before rev map:" +to_string(guard_poly));
				if(inst.tinv_ptr){
					...
					if(tgt_inv_poly_ptr){
						...
						LOGGER(DEBUG5,"discrete_post_branch_bound::post","invariant poly reverse mapped to: "+to_string(exit_set_poly));

					}
					else
						throw std::runtime_error("discrete_post: Target location invariant set type not supported\n");
					LOGGER(DEBUG7,"discrete_post_branch_bound::post","exit poly:"+to_string(exit_set_poly));
					guard_poly.add_constraints(exit_set_poly.get_constraints());
				}
				LOGGER(DEBUG5,"discrete_post_branch_bound::post","guard cons after rev map:" +to_string(guard_poly));
				/**----*/
				guard_poly.remove_redundant_constraints();
				LOGGER(DEBUG5,"discrete_post_branch_bound::post","guard constraints after redundancy check : "+to_string(guard_poly));
				...
				LOGGER(DEBUG2,"discrete_post_branch_bound::post","guard constraints list after collapsing : "+to_string(nonconst_gcons));
				...
				LOGGER(DEBUG2,"discrete_post_branch_bound::post","sfm interval(s) intersecting with the constraint poly(after rev_map): "+to_string(list));

				// construct directions set from the scenario directions choice

				/*typename sfm_cont_set<scalar_type>::vector_list
						dirs_list = nonconst_sfm->get_directions();*/
...
//					std::cout << "direction:" << vdom_vec << std::endl;
				...
				for (intv_list_type::const_iterator it = list.begin(); it
						!= list.end(); ++it) {
					LOGGER(DEBUG3,"discrete_post_branch_bound::post","treating interval: "+to_string(*it));
					LOGGER(DEBUG3,"discrete_post_branch_bound::post","treating interval of size "+to_string(it->size()+1));
					...
					typename polyhedron<scalar_type>::ptr POLY = typename polyhedron<scalar_type>::ptr(new constr_polyhedron<scalar_type>());
					// universe poly created.
					...
					for (typename math::lin_constraint_system<
						...
						LOGGER(DEBUG2,"discrete_post_branch_bound::post","constraint to intersect with sfm section : "+to_string(*itr));

						if (is_intersection_empty(sf_prov_ptr, *itr)) {
							LOGGER(DEBUG2,"discrete_post_branch_bound::post","sfm section:"+to_string(*it)+" do not intersect with the constraint\n");
							...
							LOGGER(DEBUG2,"discrete_post_branch_bound::post","sfm section contained inside the constraint\n");
						} else {
						/*Check for the alternate branch and bound algorithm*/
							 g_inters_set_ptr = support_function_provider::ptr(
...
*/
					...
					/* DEBUG */
					if (polys.is_empty()) {
						LOGGER(DEBUG3,"discrete_post_branch_bound:post:","polys list empty" );
					}
					...
					IFLOGGER(DEBUG7) {
						for (typename polyhedron_collection<scalar_type>::const_iterator
							...
							if ((*it)->is_universe()) {
								LOGGER(DEBUG7,"discrete_post_branch_bound:post:","Intersection Poly is Universe!");
							}
					...
					if (polys.size() > 1 && my_clustering > 0.0
						...
						LOGGERSW(DEBUG3,"discrete_post_branch_bound::post","clustering");
						LOGGER_ATTACH(DEBUG3,"discrete_post_branch_bound::post"," "+to_string(polys.size()) + " polyhedra using template hull, ",logger::get_last_id());
						...
						LOGGER_ATTACH(DEBUG3,"discrete_post_branch_bound::post"," obtained "+to_string(polys.size()) + " polyhedral clusters, ",logger::get_last_id());
					}
					if (!polys.is_empty()) {
						if (my_aggregation_type == 0 || double(polys.size())
							...
							if (polys.size() == 1) {
								LOGGER(DEBUG3,"discrete_post_branch_bound::post","adding " + to_string(polys.size()) + " polyhedron to result");
								ret_collection.insert(*polys.begin());
							} else {
								LOGGER(DEBUG3,"discrete_post_branch_bound::post","adding convex hull of " + to_string(polys.size()) + " polyhedra to result");
								ret_collection.insert(polys_ptr);
							}
						} else if (my_aggregation_type > 0) {
							// use template hull
							LOGGERSW(DEBUG3,"discrete_post_branch_bound::post","adding polyhedral template hull");
							LOGGER_ATTACH(DEBUG3,"discrete_post_branch_bound::post"," of "+to_string(polys.size()) + " polyhedra to result, ",logger::get_last_id());
							...
							LOGGER(DEBUG3,"discrete_post_branch_bound::post","adding " + to_string(polys.size()) + " separate polyhedra to result");
							for (typename polyhedron_collection<scalar_type>::const_iterator
									it = polys.begin(); it != polys.end(); ++it) {
								ret_collection.insert(*it);
							}
						...
						LOGGER(DEBUG3,"discrete_post_branch_bound::post","result of jump is empty");
					}
				}
				...
				LOGGER(DEBUG2,"discrete_post_branch_bound::post","computing discrete post of polyhedron");
