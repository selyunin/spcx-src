/*
 * sf_unary_template_ref.h
 *
 *  Created on: Mar 31, 2011
 *      Author: frehse
 */

#if !defined(SF_UNARY_TEMPLATE_H_) || !defined(SF_UNARY_TEMPLATE_H__BODY)
//#ifndef SF_UNARY_REF_H_
#define SF_UNARY_TEMPLATE_H_

#include "sf_set.h"

#ifndef SF_UNARY_TEMPLATE_H__BODY
#define SF_UNARY_TEMPLATE_H__BODY

namespace continuous {
namespace support_function {

/** An implicit representation of a continuous set by means of its support function,
 *  given as a const shared_ptr.
 *
 *  This differs from sf_unary in that the class doesn't need to subclass support_function_provider.
 *  It suffices to just implement compute_support.
 *  The implementor needs to, however, be a subclass of continuous_set.
 */

template<typename scalar_type, class implementor = support_function_provider> class sf_unary_template: public sf_set<scalar_type>
{
public:
	typedef boost::shared_ptr<sf_unary_template<scalar_type,implementor> > ptr;

	typedef typename sf_set<scalar_type>::affine_map affine_map;
	typedef boost::shared_ptr<const implementor> implementor_const_ptr;

	/** Construct a support function set representation from a continuous set
	 * that provides a support function. */
	explicit sf_unary_template(const implementor_const_ptr& s);

	/** Construct a support function set representation from a continuous set
	 * that provides a support function, transformed by an affine map M. */
	sf_unary_template(const implementor_const_ptr& s, const affine_map& M);

	virtual ~sf_unary_template();

	/** Shallow copy.
	 *
	 * The clone shares the support_function_provider with *this. */
	virtual sf_unary_template<scalar_type,implementor>* clone() const;
	...
	/** Returns a const pointer to the implementor object
	 *
	 * @attention This is the unmapped object. It needs to be mapped with the map
	 * returned by get_map(). */
	implementor_const_ptr get_implementor() const;
...
protected:
	...
	implementor_const_ptr my_set;
};

}
}

#include "sf_unary_template.hpp"

#endif /* SF_UNARY_TEMPLATE_H__BODY */
#endif /* SF_UNARY_TEMPLATE_H_ */
