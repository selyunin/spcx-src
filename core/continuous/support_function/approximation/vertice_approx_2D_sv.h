/*
 * vertice_approx_2D_sv.h
 *
 *  Created on: Jan 19, 2010
 *      Author: frehse
 */

#ifndef VERTICE_APPROX_2D_SV_H_
#define VERTICE_APPROX_2D_SV_H_
...
#include "core/continuous/continuous_set_exception.h"
 ...
 *
 * Requires S to prove a support vector.
 *
 ...
 * and w, |z^T (x'-x)| < eps.
 *
 * Throws a continuous_set_exception if the set is empty or unbounded.
 *
 * @todo Check error bound.
 */

template<typename scalar_type>
class vertice_approx_2D_sv {
public:
	struct sample {
		scalar_type l;
		scalar_type a;
	 ...
	 */
	vertice_approx_2D_sv(direction v, direction w,
			const support_function_provider& S, scalar_type eps) :
		my_v(v), my_w(w), my_S(S), my_eps(eps) {
		assert(math::numeric::approx_comparator<scalar_type>::is_maybe_zero(math::scalar_product(v,w)));
	}
	;

	/** Get the sf and sv at lopt. */
	sample sample(const scalar_type& lopt) {
		bool is_empty, is_bounded;
		math::vdom_vector<scalar_type> le = my_v + lopt * my_w;
		//std::cout << "opt " << my_v << "+"<< lopt << "*"<< my_w << "=" << le;
		math::vdom_vector<scalar_type> sv;
		scalar_type sopt;
		my_S.compute_support(le, sopt, sv, is_empty, is_bounded);
		if (!is_bounded)
			throw unbounded_set_exception("sv_2D_approximator: unbounded set");
		if (is_empty)
			throw empty_set_exception("sv_2D_approximator: empty set");

		scalar_type aopt = math::scalar_product(my_v, sv);
		scalar_type bopt = math::scalar_product(my_w, sv);

		//std::cout << " at " << sv << "(" << aopt << "," << bopt << ")" << std::endl;
		// insert sample after i == before j
		typename vertice_approx_2D_sv<scalar_type>::sample new_s = { lopt, aopt,
				bopt, false };
		return new_s;
	}
	;
	...
	void approx(sample_list& pts, typename sample_list::iterator& i,
		...
		while (j != iend) {
			...
			if (refine) { // no. 1
				...
				refine
						= math::numeric::approx_comparator<scalar_type>::is_definitely_strictly_larger(
								...
								lopt);
				//				std::cout << "b2>b1, l1<" << lopt << "<l2:" << refine << "; ";
			}
			if (refine) { // no. 2
				// lower bound
				sn = a1 + b1 * lopt;
			...
			if (refine) {
				...
				typename vertice_approx_2D_sv<scalar_type>::sample new_s =
						sample(lopt);
...
//				j = pts.insert(j, new_s);
				...
				// has a bug
								if (math::numeric::approx_comparator<scalar_type>::is_maybe_equal(
											b2, new_s.b) || math::numeric::approx_comparator<scalar_type>::is_maybe_equal(
													b1, new_s.b)) {
									i = j;
									++j;
								} else {
...
//std::cout << "over:" << ap << "," << bp << std::endl;
	...
	static sample_list approx(const support_function_provider& S,
		...
		if (!S.computes_support_vector()) {
			throw std::runtime_error("vertice_approx_2D_sv requires support vector");
		}
		sample_list pts;
		...
		typename vertice_approx_2D_sv<scalar_type>::sample spl;
		// 1. v=x,w=y
		{
			vertice_approx_2D_sv<scalar_type> A(x, y, S, eps);
...
//std::cout << "seed2:" << spl.a << "," << spl.b << std::endl;
		...
		}
		{
			//			std::cout << "2." << std::endl;
			// 2. v=y,w=-x
			vertice_approx_2D_sv<scalar_type> A(y, -x, S, eps);
			// reuse the last sample -> set lambda to -1
			sample_list pts2;
			spl = *pts.rbegin();
			typename vertice_approx_2D_sv<scalar_type>::sample spl2 = {
		...
		}
		{
			...
			spl = *pts.begin();
			typename vertice_approx_2D_sv<scalar_type>::sample spl3 = {
					scalar_type(-1) / spl.l, -spl.b, spl.a };
			pts2.push_back(spl3);
			i = pts2.begin();
			A.approx(pts2, i, pts2.end());
		...
		for (typename sample_list::const_iterator it = pts.begin(); it
				!= pts.end(); ++it) {
			spl = *it;
			typename vertice_approx_2D_sv<scalar_type>::sample spl2 = { spl.l,
