...
/** An implicit representation of the convex hull of two continuous sets by
 * means of their support function.
 */

template<typename scalar_type> class sf_chull: public sf_binary<scalar_type> {
public:
	typedef typename sf_binary<scalar_type>::affine_map affine_map;
	typedef boost::shared_ptr<sf_chull> ptr;
	typedef boost::shared_ptr<const sf_chull> const_ptr;

	/** Construct a support function set representation of the convex hull of
	 * two continuous sets. */
	sf_chull(const support_function_provider::const_ptr& s1,
	...
	virtual sf_chull<scalar_type>* clone() const {
		sf_chull<scalar_type>* p;
		if (this->get_map()) {
			p = new sf_chull<scalar_type> (this->my_set1, this->my_set2,
					*(this->get_map()));
		} else {
			p = new sf_chull<scalar_type> (this->my_set1, this->my_set2);
		}
	...
	virtual void print(std::ostream& os) const {
		...
		os << "chull(" << this->my_set1 << "," << this->my_set2 << ")";
		os << "}";
	}
};
