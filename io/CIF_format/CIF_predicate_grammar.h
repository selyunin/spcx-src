...
namespace ascii = boost::spirit::ascii;
    ...
    struct predicate_ast
    {
        ...
        predicate_ast& operator==(CIF_expression_ast const& exp);
        predicate_ast& operator>(CIF_expression_ast const& exp);
        predicate_ast& operator<=(CIF_expression_ast const& exp);
        predicate_ast& operator>=(CIF_expression_ast const& exp);
        predicate_ast& operator<(CIF_expression_ast const& exp);

        predicate_ast& operator*=(CIF_expression_ast const& exp); //assignment operator
    ...
    struct assignment_vector
        {
			assignment_vector(std::vector<CIF_expression_ast> const& c1
                      , std::vector<CIF_expression_ast> const& c2)
                    : left(c1), right(c2) {}
			...
			assignment_vector(){}
                   ...
                   std::vector<CIF_expression_ast> left;
                   std::vector<CIF_expression_ast> right;

                   assignment_vector& operator*=(std::vector<CIF_expression_ast> const& exp); //assignment vector operator (define right member)
         };
    ...
    struct predicate_definition : expr_definition
    {
    	...
    	qi::rule<grammar_type, std::vector<CIF_expression_ast>(), ascii::space_type > expression_vector;
    };
