...
struct predicate_tree
{
    ...
    tree::node_ptr operator()(expression_ast const& ast) {
    ...
    tree::node_ptr operator()(assignment_op const& pred)
    {
        	...
        	if (valuation_functions::variable_node* var_left = dynamic_cast<valuation_functions::variable_node*>(var.get())){
            	variable_id vid = variable::get_id_primedness_increased(var_left->my_id);
        		if (variable::get_prime_count(vid)>1) {
    			throw basic_exception(
    					"Left side of an assignment needs to be a variable. Cannot handle assignment to "
    							+ logger::formatted_to_string(variable(vid)) + ".\nNote: assignments of the form x=1, x:=1, or x'==1 are acceptable. A common mistake is x'=1.");
        		}
        		tree::node_ptr var_primed = tree::node_ptr(new valuation_functions::variable_node(vid));
    ...
    tree::node_ptr operator()(assignment_vector const& pred)
    {
    	...
    	for (std::vector<expression_ast>::size_type i = 1; i < pred.left.size(); ++i){
    ...
    tree::node_ptr operator()(std::string s)  {
        	if(s == "true" || s == "false")
        		return create_bool_const_node(s);
        	else {
...
tree::node::ptr parse_predicate(std::string expr, std::string context, const parser::parse_policy& ppol) {
	...
	if (ppol.use_symbol_table_cache) {
		symbol_table = parser::symbol_table_cache::get_symbol_table(context);
//std::cout << "using symbol stable:" << std::endl << symbol_table << std::endl;
	}
